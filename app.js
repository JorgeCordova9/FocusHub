...
// Logo Click Handler - Exit task if in workspace, otherwise do nothing
function logoClickHandler() {
  if (appData.inTaskWorkspace && appData.currentTask) {
    // Same functionality as Exit Task button
    exitTaskWorkspace();
  }
  // If on dashboard, do nothing (logo is just branding)
}
...
function switchModule(moduleName) {
  ...
  if (moduleName === 'music') {
    ...
    const musicBtn = document.querySelector('#dashboardSidebar .nav-btn[onclick*="music"]');
    ...
    const tasksBtn = document.querySelector('#dashboardSidebar .nav-btn[onclick*="tasks"]');
...
function startPomodoroPhase(isBreak) {
  ...
  if (isBreak) {
    appData.pomodoroTimer.remainingSeconds = appData.pomodoroTimer.breakTime * 60;
    appData.pomodoroTimer.totalSeconds = appData.pomodoroTimer.breakTime * 60;
  } else {
    appData.pomodoroTimer.remainingSeconds = appData.pomodoroTimer.workTime * 60;
...
function startTaskTimer(task) {
  ...
  updateTopTimerDisplay();
  updateTaskTimerControls();
  showNotification('⏱️ Timer Started: ' + Math.floor(timeRemaining) + ' minutes');
}
...
function stopTaskTimerEarly() {
  ...
  if (confirm('Stop timer and exit without completing? (Task will remain incomplete)')) {
    exitTaskWorkspace();
  }
}
...
function updateTopTimerDisplay() {
  ...
  if (appData.currentTask && appData.inTaskWorkspace) {
    ...
    if (task && appData.timer.isRunning) {
      ...
      document.getElementById('timerPhase').textContent = task.name.length > 15 ? task.name.substring(0, 15) + '...' : task.name;
      document.getElementById('timerRounds').innerHTML = `<span style="font-size: 10px; color: var(--color-text-secondary);">${Math.floor(timeRemaining)}m / ${Math.floor(task.timeAllocated)}m</span>`;
      
      // Update circular progress based on countdown
      const progress = task.timeAllocated > 0 ? (appData.timer.remaining / (task.timeAllocated * 60)) : 0;
      const strokeOffset = 163.36 - (progress * 163.36);
      document.getElementById('timerProgress').style.strokeDashoffset = strokeOffset;
      ...
      document.getElementById('timerPhase').textContent = task.name.length > 15 ? task.name.substring(0, 15) + '...' : task.name;
      document.getElementById('timerRounds').innerHTML = `<span style="font-size: 10px; color: var(--color-text-secondary);">${minutes}m / ${timeAllocatedMin}m</span>`;
      
      const progress = task.timeAllocated > 0 ? (task.timeSpent / task.timeAllocated) : 0;
      const strokeOffset = 163.36 - (progress * 163.36);
      document.getElementById('timerProgress').style.strokeDashoffset = strokeOffset;
  ...
  if (!appData.inTaskWorkspace) {
    ...
    document.getElementById('timerRounds').innerHTML = '<span style="font-size: 9px; color: var(--color-text-secondary);">Status only</span>';
  ...
  if (appData.pomodoroTimer.isActive) {
    ...
    // Update phase
    document.getElementById('timerPhase').textContent = appData.pomodoroTimer.isBreak ? 'Break' : 'Work';
    
    // Update rounds indicator
    const roundsHtml = [];
    for (let i = 0; i < appData.pomodoroTimer.rounds; i++) {
      const completed = i < appData.pomodoroTimer.currentRound;
      roundsHtml.push(`<div class="timer-round-dot ${completed ? 'completed' : ''}"></div>`);
    }
    document.getElementById('timerRounds').innerHTML = roundsHtml.join('');
    
    // Update circular progress
    const progress = (appData.pomodoroTimer.remainingSeconds / appData.pomodoroTimer.totalSeconds) * 163.36;
    document.getElementById('timerProgress').style.strokeDashoffset = 163.36 - progress;
  }
}
...
function updateFocusStats() {
  document.getElementById('sessionsToday').textContent = appData.focusStats.sessionsToday;
  document.getElementById('currentStreak').textContent = appData.focusStats.currentStreak;
}
...
function renderTabs() {
  ...
  const tabsHtml = browserState.tabs.map(tab => `
    <div class="browser-tab ${tab.id === browserState.activeTabId ? 'active' : ''}" onclick="switchTab(${tab.id})">
  ...
  tabBar.innerHTML = tabsHtml + '<button class="browser-tab-new" onclick="createNewTab()" title="New Tab">+</button>';
}
  ...
  if (tab.results && tab.results.length > 0) {
    ...
    contentArea.innerHTML = `
      <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 40px; text-align: center;">
        ...
        <p style="color: var(--color-text-secondary); margin-bottom: 8px; font-size: 16px;">Search query: "${escapeHtml(tab.query)}"</p>
...
async function performSearch(query, tabId) {
  ...
  if (tab.query && tab.query !== query) {
    ...
    tab.history.push({ query: tab.query, title: tab.title, results: tab.results });
  ...
  tab.isLoading = true;
  tab.results = [];
  tab.title = `Search: ${query.substring(0, 20)}${query.length > 20 ? '...' : ''}`;
  ...
  if (lowerQuery.includes('wikipedia') || lowerQuery.includes('wiki')) {
    results.push(
      ...
      },
      {
        ...
        description: 'The main page of the English Wikipedia. Browse millions of articles on any topic.'
      },
      {
        title: 'How Wikipedia Works',
        url: 'https://en.wikipedia.org/wiki/Wikipedia:About',
        description: 'Learn about how Wikipedia is written, edited, and maintained by volunteers worldwide.'
      }
    );
    ...
    results.push(
      {
        ...
        description: 'The official home of the Python Programming Language. Download Python, read documentation, and learn about Python development.'
      },
      {
        title: 'Python Tutorial - W3Schools',
        url: 'https://www.w3schools.com/python/',
        description: 'Learn Python programming with easy-to-follow tutorials, examples, and exercises. Perfect for beginners.'
      },
      {
        title: 'Python Documentation',
        url: 'https://docs.python.org/3/',
        description: 'Official documentation for Python 3. Comprehensive guides, library references, and tutorials.'
      }
    );
    ...
    results.push(
      {
        ...
        description: 'Comprehensive JavaScript documentation and tutorials from Mozilla Developer Network.'
      },
      {
        title: 'JavaScript.info - The Modern JavaScript Tutorial',
        url: 'https://javascript.info',
        description: 'Learn JavaScript from basics to advanced topics with clear explanations and examples.'
      },
      {
        title: 'JavaScript Tutorial - W3Schools',
        url: 'https://www.w3schools.com/js/',
        description: 'Easy to learn JavaScript tutorial with examples and exercises.'
      }
    );
    ...
    results.push(
      ...
      },
      {
        ...
        description: 'Browse millions of programming questions and answers from developers worldwide.'
      }
    );
  }
  ...
  const resultsHtml = `
    <div style="max-width: 900px; margin: 0 auto; padding: 20px;">
      <div style="margin-bottom: 24px;">
        <h3 style="color: var(--color-text); font-size: 20px; margin-bottom: 8px;">Search Results for "${escapeHtml(tab.query)}"</h3>
        <p style="color: var(--color-text-secondary); font-size: 14px;">${tab.results.length} results found • ${tab.timestamp}</p>
      </div>
        ...
        ${tab.results.map((result, index) => `
          <div style="background-color: var(--color-surface); border: 1px solid var(--color-card-border); border-radius: var(--radius-lg); padding: 20px; transition: box-shadow 0.2s, transform 0.2s;" 
            ...
            <div style="display: flex; justify-content: space-between; align-items: flex-start; gap: 16px;">
              <div style="flex: 1;">
                <a href="${result.url}" target="_blank" style="color: var(--color-primary); font-size: 18px; font-weight: var(--font-weight-semibold); text-decoration: none; display: block; margin-bottom: 8px;" 
                   onmouseover="this.style.textDecoration='underline';" 
                   onmouseout="this.style.textDecoration='none';">
                  ${escapeHtml(result.title)}
                </a>
                <p style="color: var(--color-text-secondary); font-size: 12px; margin-bottom: 8px; word-break: break-all;">${escapeHtml(result.url)}</p>
                <p style="color: var(--color-text); font-size: 14px; line-height: 1.6;">${escapeHtml(result.description)}</p>
              </div>
              <button class="btn btn--primary" onclick="window.open('${escapeHtml(result.url)}', '_blank')" style="white-space: nowrap; flex-shrink: 0;">
                Open →
              </button>
            </div>
          </div>
...
function browserGoBack() {
  const activeTab = browserState.tabs.find(t => t.id === browserState.activeTabId);
  if (!activeTab || activeTab.historyIndex < 0 || activeTab.history.length === 0) return;
  
  // Save current state to history if navigating back
  if (activeTab.historyIndex === activeTab.history.length - 1) {
    activeTab.history.push({ query: activeTab.query, title: activeTab.title, results: activeTab.results });
  }
...
function browserGoForward() {
  const activeTab = browserState.tabs.find(t => t.id === browserState.activeTabId);
  if (!activeTab || activeTab.historyIndex >= activeTab.history.length - 2) return;
...
function loadTaskNotes(task) {
  ...
  if (!task.notes || task.notes.length === 0) {
    notesList.innerHTML = '<p style="color: var(--color-text-secondary); font-size: var(--font-size-sm); padding: var(--space-12);">No notes for this task yet</p>';
    clearNoteEditor();
    return;
  }
  
  notesList.innerHTML = task.notes.map(note => `
    <div class="note-item" style="position: relative; display: flex; align-items: center; justify-content: space-between;" onclick="loadTaskNote(${task.id}, ${note.id})">
      <div style="flex: 1; min-width: 0;">
        <div class="note-item-title">${escapeHtml(note.title)}</div>
        <div class="note-item-date">${new Date(note.timestamp).toLocaleDateString()}</div>
      </div>
      <button class="note-delete-btn" onclick="event.stopPropagation(); deleteNoteFromList(${task.id}, ${note.id});" title="Delete note" style="opacity: 0; transition: opacity 0.2s; background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-sm); padding: 4px 8px; cursor: pointer; font-size: 14px; color: var(--color-text);">✕</button>
    </div>
  `).join('');
...
function loadTaskNote(taskId, noteId) {
  ...
  document.getElementById('noteTimestamp').textContent = `Created: ${date.toLocaleDateString()} ${date.toLocaleTimeString()}`;
...
function updateToolbarState() {
  ...
  commands.forEach(cmd => {
    ...
    const button = Array.from(document.querySelectorAll('.toolbar-btn')).find(btn => 
      btn.getAttribute('onclick')?.includes(cmd)
    );
    if (button) {
      if (isActive) {
...
function insertCitation() {
  ...
  const editor = document.getElementById('noteContent');
  editor.focus();
  document.execCommand('insertHTML', false, `<p><em>${escapeHtml(appData.copiedCitation)}</em></p>`);
}
...
function showDiagramPicker() {
  ...
  if (!task || !task.diagrams || task.diagrams.length === 0) {
    alert('No diagrams saved for this task. Create and save diagrams in the Whiteboard module first.');
    return;
  }
  ...
  gallery.innerHTML = task.diagrams.map((diagram, index) => `
...
function insertDiagram(index) {
  ...
  const diagramHTML = `
    <div class="diagram-wrapper" id="${diagramId}" data-width="400" data-aspect-ratio="1">
      <img src="${diagram.imageData}" style="width: 400px; height: auto;" alt="Diagram ${index + 1}">
  ...
  if (note) {
    ...
    const printContent = `
      ...
      <body>
        ...
        <div class="metadata">Created: ${new Date(note.timestamp).toLocaleString()}</div>
...
function makeImageResizable(wrapper) {
  ...
  wrapper.addEventListener('click', (e) => {
    ...
    document.querySelectorAll('.diagram-wrapper').forEach(w => w.classList.remove('selected'));
    wrapper.classList.add('selected');
  });
}
...
function renderTaskDashboard() {
  ...
  container.innerHTML = sortedTasks.map((task, index) => {
    const timeProgress = task.timeAllocated > 0 ? (task.timeSpent / task.timeAllocated) * 100 : 0;
    const timeRemaining = Math.max(0, task.timeAllocated - task.timeSpent);
    const isCompleted = task.status === 'completed';
    
    return `
      <div class="task-item ${isCompleted ? 'completed' : ''}" draggable="true" data-task-id="${task.id}" onclick="enterTaskWorkspace(${task.id})" ondragstart="handleDragStart(event)" ondragover="handleDragOver(event)" ondrop="handleDrop(event)" ondragend="handleDragEnd(event)">
        <div class="task-info">
          <div style="display: flex; align-items: center; gap: var(--space-12); margin-bottom: var(--space-8);">
            <div class="task-title ${isCompleted ? 'completed' : ''}">${isCompleted ? '✓ ' : ''}${escapeHtml(task.name)}</div>
            <span class="task-status-badge ${task.status}">${task.status === 'completed' ? '✓ Completed' : task.timeSpent > 0 ? 'In Progress' : 'Pending'}</span>
          </div>
          <div class="task-time-display">
            <span>⏱️ ${Math.round(task.timeSpent)}m / ${task.timeAllocated}m</span>
            ${!isCompleted ? `<span style="margin-left: auto;">Remaining: ${Math.round(timeRemaining)}m</span>` : `<span style="margin-left: auto; color: #00aa00;">Time Used: ${Math.round(task.timeSpent)}m</span>`}
          </div>
          <div class="task-time-bar">
            <div class="task-time-progress" style="width: ${Math.min(100, timeProgress)}%; ${isCompleted ? 'background-color: #00aa00;' : ''}"></div>
          </div>
        </div>
        <div class="task-actions">
          <button class="btn ${isCompleted ? 'btn--secondary' : 'btn--primary'}" onclick="event.stopPropagation(); enterTaskWorkspace(${task.id})">
            ${isCompleted ? 'View →' : task.timeSpent > 0 ? 'Resume →' : 'Start →'}
          </button>
          <button class="btn btn--secondary" onclick="event.stopPropagation(); deleteTask(${task.id})" title="Delete">
            🗑️
          </button>
        </div>
      </div>
...
function handleDragOver(e) {
  ...
  if (taskItem.classList.contains('task-item') && !taskItem.classList.contains('dragging')) {
...
function enterTaskWorkspace(taskId) {
  ...
  if (task.timeSpent === 0 && task.notes.length === 0 && task.diagrams.length === 0) {
...
function showTaskInitialPrompt(task) {
  ...
  document.getElementById('promptTaskName').textContent = `Welcome to "${task.name}"`;
  document.getElementById('promptTaskTime').textContent = `You have ${task.timeAllocated} minutes allocated for this task`;
...
function setupTaskResource(resourceType) {
  ...
  if (resourceType === 'notes') {
    ...
    document.querySelector('.nav-btn[onclick*="notes"]').classList.add('active');
    loadTaskNotes(task);
  } else if (resourceType === 'whiteboard') {
    document.getElementById('whiteboardModule').classList.add('active');
    document.querySelector('.nav-btn[onclick*="whiteboard"]').classList.add('active');
    loadTaskWhiteboard(task);
  }
...
function resumeTaskWorkspace(task) {
  ...
  document.querySelectorAll('.module').forEach(m => m.classList.remove('active'));
  document.querySelectorAll('#workspaceSidebar .nav-btn').forEach(b => b.classList.remove('active'));
  
  document.getElementById('browserModule').classList.add('active');
  const browserBtn = document.querySelector('#workspaceSidebar .nav-btn[onclick*="browser"]');
...
function renderDiagramGallery() {
  ...
  if (!task || !task.diagrams || task.diagrams.length === 0) {
    gallery.innerHTML = '<p style="color: var(--color-text-secondary); font-size: var(--font-size-sm); text-align: center; padding: var(--space-16);">No saved diagrams for this task yet. Draw something and click Save!</p>';
    return;
  }
...
function playPlaylist(type) {
  ...
  event.target.closest('.playlist-item').style.backgroundColor = 'var(--color-secondary)';
  event.target.closest('.playlist-item').style.border = '2px solid var(--color-primary)';
}
